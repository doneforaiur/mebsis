!#bin/bash
# Mirza Atlý, 2019. Script to setup "mebsis"
...
mezun_query="CREATE TABLE Mezun (
        ...
        FOREIGN KEY (firma_no_fk) REFERENCES Firma (ticari_$
        CONSTRAINT CHECK (gpa BETWEEN 2 AND 4)
)ENGINE=INNODB;"

takip_query="CREATE TABLE IF NOT EXISTS Follow (
        follower_id VARCHAR (7) NOT NULL,
        takip_tarihi TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        following_id VARCHAR(7) NOT NULL,
        PRIMARY KEY (follower_id, following_id),
        FOREIGN KEY (follower_id) REFERENCES Mezun (ogrenci$
        FOREIGN KEY (following_id) REFERENCES Mezun (ogrenc$
)ENGINE=INNODB;"

konusma_query="CREATE TABLE IF NOT EXISTS Konusma (
        konusma_id INT UNSIGNED NOT NULL AUTO_INCREMENT PRI$
        kullanici_bir VARCHAR(7) NOT NULL,
        kullanici_iki VARCHAR(7) NOT NULL,
        FOREIGN KEY (kullanici_bir) REFERENCES Mezun(ogrenc$
        FOREIGN KEY (kullanici_iki) REFERENCES Mezun(ogrenc$
)ENGINE=INNODB;"
mesaj_query="CREATE TABLE IF NOT EXISTS Mesaj (
        ...
        FOREIGN KEY (konusma_id_fk) REFERENCES Konusma (kon$
)ENGINE=INNODB;"
...
ilan_query="CREATE TABLE IF NOT EXISTS ilan (
        ...
        PRIMARY KEY (ilan_id),
        FOREIGN KEY (firma_fk) REFERENCES Firma (ticari_sic$
        FOREIGN KEY (ilani_acan_fk) REFERENCES Mezun (ogren$
)ENGINE=INNODB;"
...
yorum_query="CREATE TABLE IF NOT EXISTS Yorum (
        ...
        FOREIGN KEY (ogrenci_id_fk) REFERENCES Mezun (ogren$
)ENGINE=INNODB;"
